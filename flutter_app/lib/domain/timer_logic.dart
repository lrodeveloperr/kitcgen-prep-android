...
void extendTask(KitchenTask task, int nowEpochMs, Duration extension) {
  ...
  if (task.status == TaskStatus.running) {
    ...
    task.pausedRemainingMs =
        task.pausedRemainingMs! + extension.inMilliseconds;
  } else {
    task.durationSeconds =
        (task.durationSeconds ?? 0) + extension.inSeconds;
  }
}

void completeTask(KitchenTask task) {
  task.status = TaskStatus.done;
  task.deadlineEpochMs = null;
  task.pausedRemainingMs = null;
}

void skipTask(KitchenTask task) {
